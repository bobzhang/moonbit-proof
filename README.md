...
On the current branch, `moon prove` verifies **85 packages** and **894 goals**.

## Examples

The workspace now contains **85 proof packages** spanning branch reasoning,
...
- `cauchy2` through `cauchy8` prove exact Cauchy-Schwarz identities in
  dimensions 2 through 8, not just the final nonnegativity inequality.
- `lagrange4sq` proves Lagrange's four-square identity, i.e. norm
  multiplicativity for quaternion-style multiplication.
- `variance4` through `variance10` prove exact finite-dimensional variance
