...
On the current branch, `moon prove` verifies **88 packages** and **904 goals**.

## Examples

The workspace now contains **88 proof packages** spanning branch reasoning,
...
`sum_ninth_powers`, `odd_squares`, `sum_triples`,
...
`cauchy7`, `cauchy8`, `cauchy9`,
`lagrange4sq`,
`variance4`, `variance5`, `variance6`, `variance7`, `variance8`, `variance9`,
`variance10`, `variance11`

Highlights:

- `sum_cubes`, `sum_fourth_powers`, `sum_fifth_powers`,
  `sum_sixth_powers`, `sum_seventh_powers`, `sum_eighth_powers`,
  `sum_ninth_powers`, `odd_squares`, `sum_triples`, and `odd_cubes` all use
...
- `cauchy2` through `cauchy9` prove exact Cauchy-Schwarz identities in
  dimensions 2 through 9, not just the final nonnegativity inequality.
- `lagrange4sq` proves Lagrange's four-square identity, i.e. norm
  multiplicativity for quaternion-style multiplication.
- `variance4` through `variance11` prove exact finite-dimensional variance
